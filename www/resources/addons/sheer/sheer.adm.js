...
define('sheer/adm', ['jquery', 'sheer'], function(jq, sheer) {
	 ...
	 * INSTANT.SEARCH
	 */
	sheerAdm.instantSearch = (function () {
		...
		function init () {
			...
			/*
			 * Inserindo listener gerais para IS
			 */
			...
			//MAPEANDO DIGITAÇÃO NOS INPUTS
			$(document).on('keyup', '[sh-is] input:not(:hidden):not([sh-is-ignore])', function () {
				var jqNode = $(this);
				//verificando se é input com mascara de data
			...
			//MAPEANDO CHANGE EM SELECTS PADROES
			...
			//MAPEANDO CLIQUES EM CHECKBOX E RADIO
		...
		function doSearch (form, autosend) {
			...
			form.setAttribute('sh-form-loading', 'false');
			
			//EFETUANDO A BUSCA
			var isXhr = sheer.form.send(form);
		...
		function buildControls (form) {
			...
			var responseHandler = form.getAttribute('sh-form-rh') || form.getAttribute('sh-form-responseHandler') || form.getAttribute('sh-is-rh') || '[sheer/adm][instantSearch.rh]';
		...
		function rhIS (promise, node) {
			...
			function exibirLoading() {
				...
				//DETERMINANDO EXIBICAO NORMAL E CARREGANDO VARIAVEIS
				//buscando a sessão
				section = $(node).closest('section.sh-box');
				if( section.length > 0 && section.has('[sh-noLoader]') == false  ) {
					title = section.children('header').children('h1, h2, h3, h4, h5, h6').eq(0);
					if( title.length ) {
						//marcando a exibicao normal
		...
		//Resposta padrao para o InstantSearch
		return {
			rh: rhIS
		};
	})();
	...
	var _tips = (function () {
		...
		function config () {
			...
			//Caso o datePicker ainda não esteja totalmente carregado, seto um timeout para tentar novamente e finalizo
			...
			$(document.body).on('mouseenter', '[qtip]', function (evt) {
				evt.preventDefault();
	...
	var _datePicker = (function () {
		...
		function config () {
			...
			$(document).on('changeDate', 'input', function () {
				$(this).trigger('keyup');
			});

			//Inserindo listener para aplicar aos nós
			$(document).on('focus', '[datepicker], [datePicker]', function () {
				apply(this);
			});
...
define('sheer/modules/maladireta', ['sheer'], function (sheer) {
	return {
		rh : {
			sincronizarLista: function (promise, node) {
