...
abstract class AuthenticationControl {
	 ...
	 * @return boolean
	 */
	static public function authenticateUser($login, $password) {
		...
		try {
			...
			$response = \Sh\DatabaseManager::runQuery($query);
			...
			return self::authenticateUserById($user['id']);
		}
	 ...
	 * @return boolean
	 */
	static public function authenticateUserById($user) {
		...
		try {
			...
			//Recuperando usuário
			$user = \Sh\ContentProviderManager::loadContentById('user/user', $user);
			if( !$user ) {
				throw new \Sh\SheerException(array(
