...
class registrarUsuarioFacebook extends \Sh\GenericAction {
	...
	public function doAction($data) {
		...
		try {
			...
			$user = \Sh\ContentProviderManager::loadContent('social/facebook_loadByFBID', array('fbId'=>$data['fbId']));
...
// 					$imgResponse = self::copyImageFromUrl($fbPicture['url'], $imageTempPath);
			...
			$user = \Sh\ContentProviderManager::loadContentById('social/facebook', $user['id']);
			...
			if( $renovarAccessToken ) {
				...
				$user['accessToken']['expiresAt'] = \Sh\FieldDateTime::formatInputDataToSheer($accessToken['createdAt']);
	...
	static protected function copyImageFromUrl ($source, $target) {
...
class fbVincularUsuarios extends \Sh\GenericAction {
	...
	public function doAction($data) {
		...
		if( isset($data['sheerId']) && isset($data['fbId']) ) {
			...
			$user = \Sh\ContentProviderManager::loadContentById('user/user', $data['sheerId']);
			if( !$user ) {
				throw new \Sh\ActionException(array(
						'code' => null,
			...
			$facebookUser = \Sh\ContentProviderManager::loadContentById('social/facebook', $data['fbId']);
			if( !$facebookUser ) {
				//não encontrado o registrado pelo sheer vou buscar pelo id do facebook
				$facebookUser = \Sh\ContentProviderManager::loadContent('social/facebook_loadByFBID', $data);
		...
		//EMAIL
			...
			//BUSCANDO USUÁRIO SHEER
			$user = \Sh\ContentProviderManager::loadContent('user/user_lista', ['email'=>$data['email']]);
			if( $user['total'] != 1 ) {
				throw new \Sh\ActionException(array(
						'code' => null,
						'message' => 'Usuário Sheer não foi encontrado'
			...
			//BUSCANDO USUÁRIO DO FACEBOOK
			$facebookUser = \Sh\ContentProviderManager::loadContent('social/facebook_loadByEmail', $data);
			if( $facebookUser['total'] != 1 ) {
				throw new \Sh\ActionException(array(
					'code' => null,
					'message' => 'Usuário Facebook não foi encontrado'
