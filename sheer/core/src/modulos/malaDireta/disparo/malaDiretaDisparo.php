...
class dispararMalaDireta extends \Sh\GenericAction{
	...
	public function doAction($data){
		...
		if( isset($data['idAgendamento']) && $data['idAgendamento'] ){
			...
			$adicionarDisparo['idLista']  	 	= $data['idLista'];
		}
		$adicionarDisparo['enviados'] = 0;
		$adicionarDisparo['total'] = $lista['totalHabilitados'];
		...
		$this->inserirEmailsDisparadosFila($adicionarDisparo, $lista);
	...
	protected function inserirEmailsDisparadosFila ($disparo, $lista){
		...
		$emailsHabilitados = \Sh\ContentProviderManager::loadContent('malaDiretaListaEmail/emailsPorLista_ilimitado', array('idLista'=>$lista['id'], 'enviar'=>1));
		...
		foreach ($emailsHabilitados['results'] as $email){
			...
			$queryTmp .= '("'.\Sh\Library::getUniqueId().'", "'.$controlador['id'].'", "'.$disparo['id'].'", "'.$dateTimeDisparo.'", "'.addslashes($contatoInfo).'") ';
