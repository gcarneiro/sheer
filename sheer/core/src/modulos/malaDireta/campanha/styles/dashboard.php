				...
				if( $agendamentos ){
					...
					foreach ( $agendamentos as $id=>$v ){
						...
						$html.='<div>';
							...
							$html .= '<p class="sh-w-1-3">';
								$html .= '<strong>Lista(s): </strong><br />';
								
								$listas = '';
								if($v['disparoLista']){
									foreach ($v['disparoLista'] as $detalhe) {
										$listas .= ' '.$detalhe['lista']['nome'].',';
									}
								}
			...
			foreach ( $disparos['results'] as $id=>$v ) {
				...
				$html .= '<section class="sh-box"> ';
					...
					$html .= '<div class="sh-box-content">';
						...
						$html .= '<div>';
							$html .= '<p class="sh-w-1-2"><strong>Assumto:</strong><br /> '.$v['assunto'].' </p>';
							$html .= '<p class="sh-w-1-2 data-right"><strong>Lista(s) de E-mail:</strong><br /> ';
							foreach($v['disparoLista'] as $lista){
								$html .= $lista['lista']['nome'].'</br>';
							}
							$html .= '</p>';
