		...
		try {
			...
			foreach ( $maps as $idMap ) {
				...
				foreach ( $pictureMap as $name=>$nameMap ) {
					...
					//determinando caminho completo do mapper
					if( $filePicture['nameExt'] ) {
						$tmpMapper['path'] = $tmpMapper['path'].'.'.$filePicture['nameExt'];
						$tmpMapper['realPath'] = $tmpMapper['realPath'].'.'.$filePicture['nameExt'];
					}
					
					//CLONANDO IMAGEM ORIGINAL
					$tmpImage = clone $imageResource;
					
					//REDIMENSIONANDO IMAGEM CONFORME O IMAGE MAP
					//Só devo redimensionar se alguma dimensão explode a que tenho na imagem enviada, para manter o máximo
					if( ($nameMap['width'] < $imageResourceGeometry['width'] && $nameMap['width'] > 0) || ($nameMap['height'] < $imageResourceGeometry['height'] && $nameMap['height'] > 0) ) {
						...
						$bestFit = false;
						if( $nameMap['width'] > 0 && $nameMap['height'] > 0 ) {
							$bestFit = true;
						}
						$resized = $tmpImage->scaleImage($nameMap['width'], $nameMap['height'], $bestFit);
						if( !$resized ) {
					...
					if( $nameMap['processor'] ) {
						...
						if( is_subclass_of($nameMap['processor'], '\\Sh\\GenericImageProcessor') ) {
							...
							$imageProcessor = new $nameMap['processor']($tmpImage, $tmpMapper);
