...
abstract class LibraryValidation {
	...
	/**
	 * Validar um xml contra o seu xsd e guarda seus erros na variavel estatica $xmlSchemaBasedError
	 * 
	...
	static public function validateXmlFromSchema ($xmlPath, $schemaPath) {
		...
		//habilito a verificacao de erros internos
		libxml_use_internal_errors(true);
		//Aqui eu pego as mensagens de erro do xml e guardo na variavel estatica $xmlSchemaBasedError
		if (!$xml->schemaValidate($schemaPath)) {
			$errors = libxml_get_errors();
		    foreach ($errors as $error) {
		        self::$xmlSchemaBasedError .= $error->message;
		    }
		    libxml_clear_errors();
		}
		//desabilito os erros internos
		libxml_use_internal_errors(false);
		//retorno dizendo se é valido e quais seus erros
		return $validacao;
	}
	...
	/**
	 * Método para validar o Cartão Nacional do SUS
	 * 
	 * Este sabe diferenciar entre os cartões de de incio 1,2 e 7,8,9
	 * 
	 * @param string $string
	 * @return boolean
	 */
