...
abstract class Library {
	...
	/**
	 * Método para converter dataUrl capturado em forms JS e transformar para um arquivo que acabou de sofrer upload
	 * Ele espera dataUrl em baseencode64 e irá gerar um arquivo temporario com aqueles dados
	 * 
	 * @param string $dataUrl
	 * @return array($_FILES)
	 */
	static public function generateUploadedFileFromDataUrl ( $dataUrl ) {
		
		//Capturando o tipo de arquivo e o removendo
		$tmp = strpos($dataUrl, ';');
		$imageType = str_replace('data:', '', substr($dataUrl, 0, $tmp));
		$dataUrl = substr($dataUrl, $tmp+1);
		//removendo descritor da base
		$tmp = strpos($dataUrl, ',');
		$dataUrl = substr($dataUrl, $tmp+1);
		
		//Convertendo espacos para +
		$encodedData = str_replace(' ','+', $dataUrl);
  		$imagem = base64_decode($dataUrl);
  		//Gerando arquivo temporario
  		$filename = tempnam(null, null);
  		//salvando arquivo
		file_put_contents($filename, $imagem);
		
		//gerando estrutura
		$file = array(
			'name' => \Sh\Library::getUniqueId(),
			'type' => $imageType,
			'tmp_name' => $filename,
			'error'=> 0,
			'size'=> strlen($imagem)
	...
	/**
	 * Método para gerar uam string no formato "0Y 0MES 0D 0H 0M 0S" a partir de um dateTime
	 * 
	 * @param \DateTime $date
	 ...
	 * 		0Y 0MES 0D 0H 0M 0S
	 */
	static public function calculaTimeagoFromDateTime ( \DateTime $date, \DateTime $dataBase = null, $config=array(), $maxSize = null ) {
		
		//Gerando database para agora se não enviada
		if( !$dataBase ) {
			$dataBase = new \DateTime();
		}
		//Calculando diferença
		$tempo = $dataBase->diff($date);
		
		//Capturando tempos
		$data['A'] 		= (int) $tempo->format('%Y');
		$data['MES'] 	= (int) $tempo->format('%m');
		$data['D'] 		= (int) $tempo->format('%d');
		$data['H'] 		= (int) $tempo->format('%H');
		$data['M'] 		= (int) $tempo->format('%i');
		$data['S'] 		= (int) $tempo->format('%s');
		
		//Criando string do timeago
		$timeAgo = '';
		
		//Se existir um tamanho máximo definido, devo limitar à retornar apenas os maiores elementos
		//Ex: Ano > Mês > Dia, se maxSize = 2, Exibo apenas Ano e Mês e ignoro todo o resto
		//Elementos com valor = ZERO são ignorados
		if($maxSize){
			foreach($data as $id=>$e){
				if($data[$id] > 0 && $maxSize > 0){
					$maxSize--;
				}
				else{
					$data[$id] = 0;
		...
		//Processando conforme as configurações enviadas
		//Caso o usuário tenha passado as configurações desejadas devolvemos a string da forma que ele deseja
		if($config){
			foreach($config as $e){
				if(isset($data[$e])){
					$elemento = $data[$e];
